# ?? Selvaganapathy Traders - Sun Flag Fireworks, Sivakasi
### Official Diwali 2026 E-Commerce & Quotation Management Web Platform

[![Live Production](https://img.shields.io/badge/Vercel-Live%20Production-success?style=for-the-badge&logo=vercel)](https://wwwselvaganapathytradersin.vercel.app)
[![Diwali 2026](https://img.shields.io/badge/Diwali-2026%20Catalog-orange?style=for-the-badge)](https://wwwselvaganapathytradersin.vercel.app)

---

## ?? Overview
**Selvaganapathy Traders (Sun Flag Fireworks)** is a modern, high-performance web application and digital order management platform designed for wholesale and retail Diwali fireworks shopping directly from Sivakasi, Tamil Nadu.

### ?? Live Production URL
?? **[https://wwwselvaganapathytradersin.vercel.app](https://wwwselvaganapathytradersin.vercel.app)**

---

## ?? Key Features

### 1. ?? Authentic 2026 Price List (164 Products)
- **100% Pure English Corporate Catalog** covering 164 cracker varieties across 24 product categories.
- Real-time catalog filtering with category navigation pills.
- Dual-mode instant search:
  - ?? **Search by Item #** (S.No)
  - ?? **Search by Product Name / Category**

### 2. ?? Interactive Cart & Quotation System
- Live quantity increment/decrement controls with manual numeric input.
- Real-time cart calculations (varieties, total boxes, and net payable amount).
- Floating cart summary badge in the bottom corner for quick checkout access.
- Slide-out Cart Drawer and responsive Checkout Modal.

### 3. ?? 1-Click Formal e-PDF Estimate Generator
- Instant printable A4 formal estimate generation.
- Full corporate header, customer delivery details, itemized breakdown, unit rates, quantities, and authorized factory signature seal.
- Clean quotation formatting without payment blockers.

### 4. ?? Storefront & Location Map
- Factory & Storefront photo preview (`Location_Pic.jpg`).
- Embedded Google Maps location directly in the Contact Modal and Footer.
- Full festive animations and interactive background canvas fireworks.

---

## ?? Project Structure

```
www.selvaganapathytraders.in/
+-- assets/
¦   +-- app.js               # Core application engine & cart calculations
¦   +-- fireworks.js         # Canvas particle fireworks animation system
¦   +-- products.js          # Complete 164-item 2026 product database
¦   +-- qrcode.min.js        # Offline QR utility
¦   +-- index-BRggYUAu.css   # Tailored utility & component styling
+-- Location_Pic.jpg         # Storefront photograph
+-- logo.png                 # Selvaganapathy Traders brand logo
+-- index.html               # Main single-page application & modal templates
+-- package.json             # Project metadata & npm scripts
+-- vercel.json              # Vercel deployment configuration
+-- README.md                # Project documentation
```

---

## ?? Local Development

1. **Clone the repository:**
   ```bash
   git clone https://github.com/sudali599-del/selvaganapathytraders.git
   cd selvaganapathytraders
   ```

2. **Serve locally:**
   ```bash
   # Using Python
   python -m http.server 3000
   
   # Or using Node.js npx serve
   npx serve .
   ```

3. **Open in browser:**
   ```
   http://localhost:3000
   ```

---

## ?? Merchant & Factory Contact

- **Company:** Selvaganapathy Traders (Sun Flag Fireworks)
- **Address:** Main Road, Kananjampatti, Sivakasi-Vembakkottai Road, Tamil Nadu, India
- **Helplines:** +91 6383144854 / +91 99440 87728
- **Email:** selvaganapathytraders@gmail.com
- **Website:** [https://wwwselvaganapathytradersin.vercel.app](https://wwwselvaganapathytradersin.vercel.app)

---
© 2026 Selvaganapathy Traders. All rights reserved.
